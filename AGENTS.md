...
**Human documentation** (the `user_guide.md` / `admin_guide.md` guides) follows the language policy in `LANG.md` (with this sphere's selections in `LANG.local.md`): authored in the `source` language and mirrored into every `target` language. Agent-facing docs (`tech_spec.md`, the system map `.docs/architecture.md` + `.docs/architecture.local.md`) and the change timeline stay in the `source` language only.
