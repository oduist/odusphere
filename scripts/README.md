...
| Gateway | — (generated by OduSphere, not in this repo) | — |
| Frontend | — | `website/**` |
...
**Never** add a sphere's modules to `.docs/architecture.md` — document them in
the sphere-owned companion. That is what keeps updates conflict-free (the
contract in `ODUSPHERE.md` §6 enforces this for the system map).
