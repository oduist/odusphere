...
1. **HUMAN LAYER (For Humans — two audiences):**
   ...
   * **Languages — governed by `LANG.md` (policy) + `LANG.local.md` (this sphere's selections):** the human guides are authored in the `source` language defined in `LANG.local.md` and **mirrored into every `target` language** under `<module>/doc/i18n/<lang>/`, in the **same commit** (or via the `odu-doc-i18n` skill). The Userbook/Adminbook serve each reader the file matching their Odoo language and fall back to the source when a translation is absent. Only the human guides are translated — the technical SPEC, the system map, and the change timeline stay in the `source` language. Once a target language exists, authoring documentation in **all** target languages is part of the Definition of Done. (UI strings of modules are localised separately, the Odoo `.po`/`.pot` way.)
